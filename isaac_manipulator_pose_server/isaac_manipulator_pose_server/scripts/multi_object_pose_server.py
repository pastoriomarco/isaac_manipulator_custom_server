...
class MultiObjectPoseServer:
    """Persistent service server for multi-object pose scans."""
    ...
    def __init__(self, config_file: str):
        ...
        rclpy.init()
        self._node = rclpy.create_node('multi_object_pose_server')
        self._pipeline = BinObjectPosePipeline(node=self._node, config=self._config)
    ...
    def spin(self):
        self._node.get_logger().info('multi_object_pose_server is ready.')
        self._executor.spin()
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description='Persistent server for multi-object bin pose scans.')
    parser.add_argument(
        '--config-file',
        type=str,
...
def main():
    ...
    server = MultiObjectPoseServer(config_file=args.config_file)
    try:
        server.spin()
